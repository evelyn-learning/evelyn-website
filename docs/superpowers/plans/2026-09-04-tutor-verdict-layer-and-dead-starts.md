...
- **The declared fraction gap, and metaphor churn as the response.** Amisha said she could not do fractions four times (180.5s, 389.6s, 1170.7s, 1250.0s) and twice asked to have it written down. **The gap WAS recorded correctly** — `flag_prerequisite_gap` fired at 00:46:08Z, 26s after "I know nothing when it comes to fractions", producing `conceptLabel: "fraction fluency (LCD, clearing denominators)"`, `status: confirmed`, `confidence: 0.75`, signals `[STUDENT_VERBALIZED_CONFUSION, NO_RECOVERY, SLOW_SEGMENT]`, with her verbatim quote and a `conceptId`. (⚠️ It lives on `studentprofiles` keyed by **`externalStudentId`**; the session doc's `weakTopics: []` is NOT the gap store and reading it as such gives the opposite conclusion.)

  What failed is everything after detection. `firstSeenAt == lastSeenAt` — it fired **once** and never re-stamped while she declared it twice more over the following 13 minutes, and the in-session response was four different analogies for one LCM idea in 110s (market 194.5s, ticket blocks 242.2s, coupon 307.6s, store price tag 524.7s), continuing *after* she said the words themselves were the problem (302.9s/304.6s: *"Why do I keep getting confused with words? … Because it's words."*). Nothing in the engine could have detoured her either: per the 2026-09-03 audit, `plan.prerequisites` never reaches the prompt, there is no concept→plan resolver, swaps are hard-filtered by grade, and `onProposePlanSwap` is unwired in the embed page.

  **Deliberately not a task here.** Praveen parked this for a dedicated pedagogy round in a fresh session after these fixes ship, with a wider brief: detect gaps by **inference** rather than waiting for a student to name them (Amisha is the unrepresentative case), and remediate at three triggers — JIT, mid-session recurrence, and next-session start — the last two as a consent-gated offer rather than a silent detour. Design intent is captured in the `project_tutor_inferred_gaps_and_consented_recap` memory; the dependency is the cross-plan-navigation build order in `project_tutor_cross_plan_navigation_gaps`.
