...
- [x] Live-test round for Phase 3 as a unit; ship; record in ledger. **SHIPPED to prod 2026-07-23** (engine main @ 60b8dd53, ./deploy-update.sh, flags NEXT_PUBLIC_TUTOR_TTS_WS=on + NEXT_PUBLIC_TUTOR_RENDER_WORD_ANCHOR=on added to prod .env.local — DRAW_ON was already on). Four dev live rounds: R1 caught 4ac-as-word + numeric-answers-dropped (5ab1179d); R2 caught markdown-in-labels (860135be) + documented false-praise/marker-point-drops; R3 caught praise-momentum beating the prompt rule → per-turn verdict guard, and reanchor-to-drain holds → REANCHOR_MAX_HOLD_SENTENCES=2 (92034f82+60b8dd53); R4 (session-1784770536318) PASS — user-confirmed render timing + verdict behavior, 369 word ticks 0 fallbacks. Post-deploy verified: prod token mints {tts,stt}+voice_substitutions, TTS 200, site up.
