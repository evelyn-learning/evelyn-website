...
export type WhiteboardCommand =
  ...
  | { action: 'showStats'; title?: string; type: 'histogram' | 'boxplot' | 'dotplot' | 'bar' | 'pie' | 'distribution' | 'scatter' | 'scatterplot' | 'scatterplot_regression'; data?: number[]; bins?: Array<[number, number, number]>; showCounts?: boolean; binWidth?: number; xLabel?: string; yLabel?: string; boxplot?: { datasets: BoxPlotData[]; showValues?: boolean }; bar?: { categories: string[]; values: number[]; colors?: string[] }; pie?: { slices: PieSlice[]; showPercentages?: boolean }; distribution?: { family: 'normal' | 't' | 'chi-square' | 'F'; params?: { mean?: number; sd?: number; df?: number; df1?: number; df2?: number }; shade?: { type: 'less' | 'greater' | 'between' | 'outside'; a?: number; b?: number; color?: string }; xRange?: [number, number]; showMean?: boolean; probabilityLabel?: string }; points?: Array<{ x: number; y: number; label?: string }>; regression?: { slope: number; intercept: number }; showTrendLine?: boolean; rValue?: number; rSquared?: number; equationLabel?: string; highlightPoint?: { x: number; y: number; label?: string }; showResiduals?: boolean }
  ...
  | { action: 'showLabeledImage'; spec: unknown }
