/**
 * Exercise the REAL Haiku doodler on one representative concept per Wave-4
 ...
 *
 * Usage: npx tsx scripts/sketch-primitives-wave4-doodler.ts
 * Output: scratchpad/primitives-wave4-doodler.png
 */
...
const PROBES: { primitive: string; concept: string; labels: string[] }[] = [
  { primitive: 'coordinate_grid', concept: 'a point plotted on an xy grid', labels: [] },
  { primitive: 'orbit', concept: 'a planet orbiting the sun', labels: ['sun', 'planet'] },
  { primitive: 'molecule', concept: 'a water molecule H2O', labels: [] },
  { primitive: 'bar_compare', concept: 'compare the heights of three bars', labels: [] },
];
...
(async () => {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    <h1>Wave-4 primitives — real Haiku doodler output</h1>
    <div class="grid">${cards.join('')}</div>
  </body></html>`;

  const htmlPath = path.join(OUT_DIR, 'primitives-wave4-doodler.html');
  fs.writeFileSync(htmlPath, html);
  const browser = await chromium.launch();
  const page = await browser.newPage({ viewport: { width: 1000, height: 1200 }, deviceScaleFactor: 2 });
  await page.goto('file://' + htmlPath);
  await page.waitForTimeout(200);
  const pngPath = path.join(OUT_DIR, 'primitives-wave4-doodler.png');
