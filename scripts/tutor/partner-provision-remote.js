#!/usr/bin/env node
/**
 * partner-provision-remote.js — generalised from kanzoo-provision-remote.js
 * (2026-09-22, GreenApple pilot). Runs ON the production box, from
...
const LEAD_NAME_PATTERN = process.env.LEAD_NAME_PATTERN || '';   // empty = skip the sandbox-lead step
// Optional JSON object merged into the partner row's flagOverrides on create,
// and $set onto an EXISTING row on --write (idempotent re-runs). GreenApple:
// {"practice_assign_cap":3} — see practice-assign/assign.ts capForPartner().
let FLAG_OVERRIDES = {};
if (process.env.FLAG_OVERRIDES) {
  try { FLAG_OVERRIDES = JSON.parse(process.env.FLAG_OVERRIDES); } catch (e) { die('FLAG_OVERRIDES is not valid JSON'); }
  if (!FLAG_OVERRIDES || typeof FLAG_OVERRIDES !== 'object' || Array.isArray(FLAG_OVERRIDES)) die('FLAG_OVERRIDES must be a JSON object');
}
...
(async () => {
  ...
  // ---- 1. test sandbox lead (only when LEAD_NAME_PATTERN is set) ------------
  const leads = db.collection('tutorsandboxrequests');
  if (LEAD_NAME_PATTERN) {
  ...
  for (const m of remaining) console.log(`    ${m.name} · ${m.email} · ${m.company} · ${m.status} · ${m.createdAt?.toISOString?.() ?? m.createdAt}`);
    ...
    console.log('\n[1] sandbox-lead step skipped (LEAD_NAME_PATTERN not set)');
  }
  ...
  if (existing) {
    ...
    if (Object.keys(FLAG_OVERRIDES).length) {
      const merged = { ...(existing.flagOverrides || {}), ...FLAG_OVERRIDES };
      console.log(`    flagOverrides now=${JSON.stringify(existing.flagOverrides || {})} → would set ${JSON.stringify(merged)}`);
      if (WRITE) {
        await partners.updateOne({ _id: PARTNER_ID }, { $set: { flagOverrides: merged, updatedAt: new Date().toISOString() } });
        console.log('    flagOverrides updated (registry cache TTL / restart applies before the engine sees it)');
      }
    }
  } else {
