...
console.log('process-tool-call: Phase 29 — eye cross-section');
check('show_diagram eye_cross_section (bare) → ok', () => {
  const r = processToolCall('show_diagram', { type: 'eye_cross_section', params: {} });
  assert.equal(r.ok, true);
  if (r.ok) assert.equal(r.command.action, 'showDiagram');
});
check('show_diagram eye_cross_section highlight + alias → ok', () => {
