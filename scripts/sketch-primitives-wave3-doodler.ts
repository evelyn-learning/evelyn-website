/**
 * Exercise the REAL Haiku doodler on one representative concept per Wave-3
 ...
 *
 * Usage: npx tsx scripts/sketch-primitives-wave3-doodler.ts
 * Output: scratchpad/primitives-wave3-doodler.png
 */
...
const PROBES: { primitive: string; concept: string; labels: string[] }[] = [
  { primitive: 'pulley', concept: 'a pulley lifting a load', labels: ['load', 'pull'] },
  { primitive: 'lever', concept: 'a lever balanced on a fulcrum', labels: ['load', 'effort', 'fulcrum'] },
  { primitive: 'gauge', concept: 'a speedometer at 70%', labels: ['speed'] },
  { primitive: 'axis', concept: 'a number line from 0 to 10', labels: ['0', '10'] },
];
...
(async () => {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    <h1>Wave-3 primitives — real Haiku doodler output</h1>
    <div class="grid">${cards.join('')}</div>
  </body></html>`;

  const htmlPath = path.join(OUT_DIR, 'primitives-wave3-doodler.html');
  fs.writeFileSync(htmlPath, html);
  const browser = await chromium.launch();
  const page = await browser.newPage({ viewport: { width: 1000, height: 1200 }, deviceScaleFactor: 2 });
  await page.goto('file://' + htmlPath);
  await page.waitForTimeout(200);
  const pngPath = path.join(OUT_DIR, 'primitives-wave3-doodler.png');
