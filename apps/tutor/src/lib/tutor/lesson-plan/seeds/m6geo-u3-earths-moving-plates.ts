 ...
 * THAT the continents were once joined. It never says what happens at the
 * different kinds of plate edge. The three plate-boundary types (convergent,
 * divergent, transform), what each one builds, and the hazard-versus-disaster
 * distinction are Grade 7 (`m7geo-u2-plate-tectonics-and-natural-hazards.ts`)
 * and must not appear here. Naming that earthquakes and volcanoes cluster
 * ALONG plate edges is a locating fact and is in scope; explaining WHICH kind
 * of edge produces WHICH result is not.
 *
 * DEPTH CEILING NOTE FOR THE FAN-OUT: read every keyIdea below and notice what
 * is missing. There is no mechanism, no closed typology, no causal chain
 * longer than one link. That plainness is deliberate and is the single thing
 * this course is held to. Grade 6 sentences are supposed to look too simple
 * next to the Grade 7 file on the same subject.
 *
 ...
 * full wrong reason rather than a short wrong label, so the correct choice is
 * the strictly longest in exactly one of the three items, and the three
 * correct choices sit at three different ids.
 *
...
export const SEED_M6GEO_U3_EARTHS_MOVING_PLATES: LessonPlan = {
  ...
  segments: [
    ...
    },
    {
      ...
      keyIdeas: [
        "EARTH'S OUTER SHELL IS BROKEN INTO PIECES, NOT ONE UNBROKEN SKIN. The rocky outside of Earth -- the crust and the very top of the layer underneath it -- is cracked into large slabs called TECTONIC PLATES. There are about a dozen big ones and a number of smaller ones. A plate is not the same thing as a continent. Some plates carry a continent, some carry nothing but ocean floor, and many carry a piece of each. The whole surface of Earth sits on a plate, including the bottom of the sea.",
        'THE PLATES MOVE, AND THEY MOVE ABOUT AS FAST AS YOUR FINGERNAILS GROW. That is a few centimeters a year. Nobody feels it and no map of the world has to be redrawn because of it. But a few centimeters a year, kept up for millions of years, adds up to thousands of kilometers, and that is how a slab of rock crosses an ocean.',
    ...
    },
    {
      ...
      steps: [
        ...
        'Look for evidence of a completely different kind. Fossils of the same ancient animals have been found on the coast of South America and on the coast of southern Africa. Those animals could not have swum across an entire ocean, so the simplest explanation is that the two coasts were once one piece of ground.',
    ...
    },
    {
      ...
      steps: [
        ...
        'WRONG: "the plates race around." CORRECT: "the plates creep, at about the speed your fingernails grow." Then check the other end of the scale, because this is the part that matters: keep a few centimeters a year going for millions of years and the total is thousands of kilometers. Slow is not the same as still.',
      ],
      answer:
    ...
    },
    {
      ...
      kind: 'try_yourself',
      problem:
        'Fossils of the same ancient animals have been found on the east coast of South America and on the west coast of southern Africa, and bands of rock on the two coasts match each other. What do geographers conclude from this?',
      responseFormat: 'mcq',
      choices: [
    ...
    },
    {
      ...
      mustRemember: [
        ...
        'The plates move a few centimeters a year, about as fast as fingernails grow. That is far too slow to feel and far too slow to change a world map, but over millions of years it moves a continent across an ocean.',
