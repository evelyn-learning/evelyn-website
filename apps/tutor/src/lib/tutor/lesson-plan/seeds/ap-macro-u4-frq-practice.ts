...
export const SEED_AP_MACRO_U4_FRQ_PRACTICE: LessonPlan = {
  ...
  segments: [
    ...
    },
    {
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1, scoringCriteria: 'Computes the money multiplier as 1 / RR = 1 / 0.08 = 12.5.', modelResponse: 'Money multiplier = 1 / RR = 1 / 0.08 = 12.5.' },
        { criterionId: 'b', maxPoints: 2, scoringCriteria: 'Sets up max ΔM = multiplier × Fed purchase and computes 12.5 × $40M = $500M (1 point for setup, 1 for the correct number).', modelResponse: 'Maximum increase in the money supply = multiplier × Fed purchase = 12.5 × $40M = $500M.' },
        { criterionId: 'c', maxPoints: 3, scoringCriteria: 'Shows the first-round T-account for the selling bank: assets — bonds down $40M, reserves up $40M (net zero); liabilities unchanged; and identifies the $40M as excess reserves available for new lending.', modelResponse: 'ASSETS: bonds DOWN $40M; reserves UP $40M (the Fed paid in newly created reserves) — total assets unchanged. LIABILITIES: unchanged. The bank now holds $40M of EXCESS reserves (no new deposits back them), which it can lend out to start the multiplier cascade.' },
        { criterionId: 'd', maxPoints: 3, scoringCriteria: 'Identifies TWO specific real-world dampers with mechanisms — e.g., cash leakage (borrowers hold cash rather than redeposit), banks choosing to hold excess reserves, or weak loan demand (no creditworthy borrowers).', modelResponse: '(i) CASH LEAKAGE — borrowers keep some loan proceeds as physical cash instead of redepositing, so those dollars exit the multiplier cascade. (ii) EXCESS RESERVES — banks may choose to hold more than required, especially in recessions when lending feels risky, so less is lent at each round. (Also acceptable: weak loan demand — banks cannot lend without creditworthy borrowers.)' },
      ] },
      responseFormat: 'frq',
      hints: [
    ...
    },
    {
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1, scoringCriteria: 'Identifies that the MONEY SUPPLY (Ms) shifts RIGHTWARD because the Fed\'s bond purchase injects reserves.', modelResponse: 'The money supply (Ms) shifts RIGHTWARD — the Fed\'s open-market purchase injects reserves, and the money multiplier expands the money supply.' },
        { criterionId: 'b', maxPoints: 1, scoringCriteria: 'States that the equilibrium NOMINAL interest rate FALLS (Ms right, Md unchanged, new intersection at lower i).', modelResponse: 'The equilibrium nominal interest rate FALLS: with Ms shifted right and Md unchanged, the new intersection is at a lower i.' },
        { criterionId: 'c', maxPoints: 4, scoringCriteria: 'Traces the full transmission chain — lower nominal i → lower real r → investment rises (and interest-sensitive C) → AD shifts RIGHT — and states the AD-AS effects: real GDP rises, price level rises, cyclical unemployment falls. Skipped links lose points.', modelResponse: 'Lower nominal i → lower real r (with stable inflation expectations) → INVESTMENT RISES as more capital projects become profitable, and interest-sensitive consumption rises modestly. Higher C and I shift AGGREGATE DEMAND RIGHTWARD. At the new short-run equilibrium, real GDP RISES, the price level RISES, and cyclical unemployment FALLS — the recessionary gap narrows.' },
        { criterionId: 'd', maxPoints: 3, scoringCriteria: 'Computes the spending multiplier 1/(1−0.8) = 5, the AD shift 5 × $25B = $125B, and concludes the $200B gap is NOT fully closed ($75B remains) — 1 point each.', modelResponse: 'Spending multiplier = 1 / (1 − 0.8) = 5. AD shift = 5 × $25B = $125B. The gap was $200B, so it is NOT fully closed — a $75B recessionary gap remains.' },
      ] },
      responseFormat: 'frq',
      hints: [
    ...
    },
    {
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1, scoringCriteria: 'Identifies that DEMAND for loanable funds shifts RIGHTWARD because the government must borrow the $300B, adding to private demand.', modelResponse: 'The DEMAND for loanable funds shifts RIGHTWARD — deficit financing means the government borrows $300B in the loanable funds market, adding to existing private demand.' },
        { criterionId: 'b', maxPoints: 1, scoringCriteria: 'States that the equilibrium REAL interest rate RISES (D right, S unchanged).', modelResponse: 'The equilibrium real interest rate RISES: with supply unchanged and demand shifted right, the new intersection is at a higher r.' },
        { criterionId: 'c', maxPoints: 1, scoringCriteria: 'States that private investment FALLS because the higher real rate makes some projects unprofitable.', modelResponse: 'Private investment FALLS — at the higher real interest rate, some private investment projects are no longer profitable.' },
        { criterionId: 'd', maxPoints: 3, scoringCriteria: 'Explains that the net AD shift = (spending multiplier × ΔG) MINUS the private-investment reduction from the higher real rate, and names this offset as crowding out.', modelResponse: 'Total ΔAD = (spending multiplier × ΔG) − (the reduction in private I caused by the higher real rate). The stimulus shifts AD right through G, but the rate rise suppresses I, partially offsetting the shift. This offsetting I reduction is CROWDING OUT, so the actual AD shift is smaller than the multiplier alone implies.' },
        { criterionId: 'e', maxPoints: 3, scoringCriteria: 'Gives a LARGE crowding-out scenario (full employment / tight market / steep supply of loanable funds, so rates rise sharply) and a SMALL one (deep recession / flat supply with idle savings, so rates barely move), each justified via the supply-curve elasticity or state of the economy.', modelResponse: 'LARGE: at FULL EMPLOYMENT with a steep loanable-funds supply curve, the market is already tight — added government borrowing drives rates up sharply and I falls substantially. SMALL: in a DEEP RECESSION with a flat supply curve and abundant idle savings, government borrowing barely moves rates, so I is barely affected — which is why fiscal stimulus is most potent in deep recessions.' },
      ] },
      responseFormat: 'frq',
      hints: [
