 ...
 *  Fix round 1 (latency): the calls run in parallel under a budget
 *  (topUpBudgetMs(): PRACTICE_EMIT_TOPUP_BUDGET_MS, default 25 s — final fix
 *  wave M2: 12 s was below a typical Sonnet generate + verify). This runs
 *  inside the session-result emit; the academy's End path fires it without
 *  waiting and its sweep allows 45 s, so the budget only delays the homework
 *  card, never the student. On timeout we keep what is retrieved
...
/** The top-up budget: PRACTICE_EMIT_TOPUP_BUDGET_MS when a positive integer,
 *  else TOP_UP_BUDGET_MS. Read per call (no restart-order surprises). Pure. */
export function topUpBudgetMs(raw: string | undefined = process.env.PRACTICE_EMIT_TOPUP_BUDGET_MS): number {
  const n = Number(raw);
  return raw !== undefined && raw.trim() !== '' && Number.isInteger(n) && n > 0 ? n : TOP_UP_BUDGET_MS;
}
