...
export const DIAGRAM_CATALOG: DiagramKindMeta[] = [
  ...
  { kind: 'comparison_table', displayName: 'Comparison Table', whenToUse: 'Compare items across attributes in a grid. Items become column headers; attributes become row labels. Prefer 3 items (columns) or fewer — a wide table shrinks to stay legible and, past that, only scrolls into view rather than growing the board. For 4+ items to compare, use a stacked list per item, or fall back to t_chart for exactly 2.', subjects: ['general'], grades: { from: 3, to: 12 }, paramSchema: 'items:[string] (column headers, N entries), attributes:[string] (row labels, M entries), cells:string[M][N] (outer length = attributes.length; each inner row has items.length entries — one cell per item — read as cells[attributeIndex][itemIndex]), title?' },
  { kind: 't_chart', displayName: 'T-Chart', whenToUse: 'Two-column comparison (pros/cons, before/after, etc.). For a 2-way comparison, prefer this over comparison_table.', subjects: ['general'], grades: { from: 2, to: 12 }, paramSchema: 'leftHeader, rightHeader, leftItems:[string], rightItems:[string], title?' },
