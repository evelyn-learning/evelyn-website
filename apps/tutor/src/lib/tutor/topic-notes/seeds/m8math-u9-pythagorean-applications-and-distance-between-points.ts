...
export const BASELINE_M8MATH_U9_PYTHAGOREAN_APPLICATIONS_AND_DISTANCE_BETWEEN_POINTS: TopicNotesBaseline = {
  ...
  pointers: [
    ...
    { content: `The hypotenuse is **always** the longest side. If your answer is longer than the side labeled as the hypotenuse, you added when you should have subtracted (or vice versa). Check the window: hypotenuse > any leg, hypotenuse < both legs added together.`, kind: 'tip' },
    { content: `When you subtract coordinates to find Δx or Δy, a negative answer means you subtracted in the wrong order. Flip it to make it positive—legs are lengths, never negative. Example: from x = -2 to x = 4 is 4 - (-2) = 6, not 2.`, kind: 'common-error' },
    { content: `**Do not split a square root across a plus sign.** √(a² + b²) is NOT √a² + √b². Always finish the addition under the root first, then take the square root. This is why 6 + 4 ≠ √52.`, kind: 'gotcha' },
    { content: `For a box's space diagonal, carry the **squared** bottom diagonal into the second triangle—don't take its square root yet. If the bottom diagonal squared is 25, use 25 in the next step, not 5. You only take one square root at the very end.`, kind: 'common-error' },
    { content: `Name the sides **before** you plug numbers into a² + b² = c². Identify which side is the hypotenuse (across from the right angle), which two are legs, and which one is unknown. A ladder against a wall is the hypotenuse; the height it reaches is a leg.`, kind: 'vocab-note' },
    { content: `When a square root is not a perfect square, trap it between two consecutive whole numbers, then test **one decimal place** to estimate. Example: 6² = 36 and 7² = 49, so √45 is between 6 and 7; test 6.7² and 6.8² to narrow it down.`, kind: 'tip' },
    { content: `Counting diagonal grid squares is **not** the same as the straight-line distance. A diagonal square has hypotenuse √2 ≈ 1.4, not 1. Always draw the right triangle with horizontal and vertical legs, never count the diagonal steps.`, kind: 'edge-case' },
    { content: `A hypotenuse is always **shorter** than the sum of the two legs. If your answer equals or exceeds (leg₁ + leg₂), you've made a mistake. Use this window as a final sanity check on every problem.`, kind: 'tip' },
  ],
};
