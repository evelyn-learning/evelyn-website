...
export const BASELINE_M6SCI_U10_POPULATION_GROWTH_AND_RESOURCE_DEMAND: TopicNotesBaseline = {
  ...
  pointers: [
    ...
    { content: `Total demand = population × per-person use. This is multiplication, not addition. Doubling the population while keeping per-person use the same always doubles the total demand — never stays flat.`, kind: 'vocab-note' },
    { content: `Don't assume one factor 'wins' without doing the math. A big population increase can outweigh a small drop in per-person use, OR a huge drop in per-person use can outweigh population growth. You must multiply both current numbers and compare to the old total.`, kind: 'common-error' },
    { content: `Population growing while per-person use stays the same still raises total demand. Each person is not using more, but there are more people, so the product goes up. This is the core trap this lesson warns against.`, kind: 'gotcha' },
    { content: `A renewable resource can become nonrenewable because demand rose, not because the resource changed. If an aquifer recharges at a fixed rate but total water use climbs past that rate, pumping that used to be sustainable is now not — the aquifer itself did not shrink.`, kind: 'edge-case' },
    { content: `The same multiplication (population × per-person amount) works for waste sent to a landfill or any footprint, not just resources drawn from nature. More people, same trash per person = more total trash reaching the landfill.`, kind: 'tip' },
    { content: `Per-person use dropping is good for the environment, but it does not automatically drop total demand if population is growing fast enough. Good habits alone do not guarantee lower total impact — the population number matters just as much.`, kind: 'gotcha' },
  ],
};
