...
export async function exportTutorSessionPDF(
  ...
  sessionGoal: string,
  studentName?: string,
  options?: { includeDebugData?: boolean; subject?: string; level?: string; productName?: string }
): Promise<void> {
  const includeDebug = options?.includeDebugData ?? false;
  // GreenApple pilot (task 13, spec §8.5): a partner PDF must carry no
  ...
  pdf.setFontSize(10);
  pdf.setFont('helvetica', 'normal');
  pdf.text(sanitizeForPDF(`Generated by ${brandName}`), margin, 24);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    pdf.setFontSize(7);
    pdf.setTextColor(156, 163, 175);
    pdf.text(sanitizeForPDF(`Generated by ${brandName} -- AI Voice Tutor`), margin, 287);
