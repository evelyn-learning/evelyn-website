...
assert.equal(countBoardRenderTools(['some_future_tool']), 1);
assert.equal(countBoardRenderTools(['set_current_problem']), 0);             // homework rail pointer — state, not ink
assert.equal(countBoardRenderTools(['show_problem', 'set_current_problem']), 1);              // unknown ⇒ counted (fail toward repair)
