...
/**
 * The presenter/teleprompter window (NAHQ kit, generalised) — mirrors Google Slides' presenter
 ...
 * shared. Stays in sync with the deck over BroadcastChannel(config.channel)
 * and can drive it directly with its own arrow keys / buttons.
 */
export default function PresenterView({ config }: { config: DeckConfig }) {
  const SLIDES = config.slides;
  const SPEAKER_NOTES = config.notes;
  ...
  const { postIndex, postHello } = useDeckChannel(config.channel, (msg) => {
    if (msg.type === "hello") {
      postIndex(index);
