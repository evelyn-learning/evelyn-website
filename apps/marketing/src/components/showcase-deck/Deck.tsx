...
import { useDeckChannel } from "./deckSync";
...
export default function Deck({ config, onSlideView }: { config: DeckConfig; onSlideView?: (index: number) => void }) {
  ...
  // Presenter-view sync (BroadcastChannel(config.channel)). The deck is the
  ...
  const { postIndex } = useDeckChannel(config.channel, (msg) => {
    if (msg.type === "hello") {
      postIndex(index);
  ...
  return (
    <div className="deck-root relative h-screen w-screen overflow-hidden bg-slate-950">
      {SLIDES.map((s, i) => {
        const Slide = s.Component;
        return (
          <div
            key={s.id}
            className={`deck-slide-page h-full w-full ${
              i === index ? "deck-slide-active" : ""
            }`}
          >
            <Slide />
          </div>
      ...
      <button
        ...
        onClick={goPrev}
        disabled={index === 0}
        className="deck-chrome group absolute inset-y-0 left-0 z-20 w-1/3 cursor-w-resize disabled:cursor-default"
      />
      <button
        aria-label="Next slide"
        onClick={goNext}
        disabled={index === total - 1}
        className="deck-chrome group absolute inset-y-0 right-0 z-20 w-1/3 cursor-e-resize disabled:cursor-default"
      />
      ...
      {/* Presenter-view hint — first slide only, unobtrusive corner link.
          ...
          rel="opener" (same origin) keeps the opener relation so the browser
          copies sessionStorage — the new tab opens already unlocked. */}
      {index === 0 && (
        <a
          href={config.presenterPath}
          target="_blank"
          rel="opener"
          className="deck-chrome pointer-events-auto absolute right-5 top-5 z-30 text-xs text-slate-600 transition-colors hover:text-slate-400"
        >
          Presenter view
        </a>
      )}
      ...
      <div className="deck-chrome pointer-events-none absolute inset-x-0 bottom-0 z-30 flex items-center justify-between px-6 pb-5">
        <button
          aria-label="Previous slide"
          onClick={goPrev}
          disabled={index === 0}
