...
- an analogy that is secretly a GRAPH — a trail/hill/path with an average-slope or tangent line, a quantity rising and falling over time or distance, anything whose meaning lives in a curve plus straight lines with particular SLOPES. Freehand strokes cannot make two slopes genuinely equal or a line genuinely tangent, so the picture asserts false math; the tutor has a precise graph tool for these.
