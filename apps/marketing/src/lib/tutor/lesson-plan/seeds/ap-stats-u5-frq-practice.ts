...
export const SEED_AP_STATS_U5_FRQ_PRACTICE: LessonPlan = {
  ...
  segments: [
    ...
    { id: 'try-frq-prop', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1.5, scoringCriteria: 'States Random (SRS) is satisfied; checks Large Counts using the claimed p = 0.75 (np ≥ 10 AND n(1−p) ≥ 10); states the 10% condition (population of grads > 800).', modelResponse: 'Random: SRS ✓. Large Counts (assuming claim p = 0.75): np = 80(0.75) = 60 ≥ 10 ✓; n(1−p) = 80(0.25) = 20 ≥ 10 ✓. 10%: assume school has > 800 grads. ✓' },
        { criterionId: 'b', maxPoints: 3, scoringCriteria: 'Computes μ_p̂ and σ_p̂ using the claimed p; standardizes p̂ = 0.65 to a z-score; finds P(Z ≤ z) via a correct normal-distribution calculation.', modelResponse: 'μ_p̂ = 0.75, σ_p̂ = √(0.75·0.25/80) = √(0.002344) ≈ 0.0484. z = (0.65 − 0.75)/0.0484 ≈ -2.066. P(Z ≤ -2.066) ≈ 0.0194.' },
        { criterionId: 'c', maxPoints: 2, scoringCriteria: 'Interprets the small probability as strong evidence against the claim, tied to the computed value; concludes the true college-attendance rate is likely LESS than 75%.', modelResponse: 'The probability of getting p̂ ≤ 0.65 is only about 1.9% if the school\'s claim were true. This is unusual evidence — the true graduation-to-college rate is likely LESS than 75%.' },
      ] },
    ...
    { id: 'try-frq-mean', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 2, scoringCriteria: 'States μ_x̄ = μ = 30 and computes σ_x̄ = σ/√n ≈ 1.697; justifies approximate Normality via the CLT since n = 50 ≥ 30, despite the population being right-skewed.', modelResponse: 'μ_x̄ = 30. σ_x̄ = 12/√50 ≈ 1.697. n = 50 ≥ 30 → CLT applies. x̄ ~ N(30, 1.697) approximately.' },
        { criterionId: 'b', maxPoints: 2.5, scoringCriteria: 'Standardizes x̄ = 33 to a z-score using σ_x̄ from part (a); computes P(Z > z) correctly.', modelResponse: 'z = (33 − 30)/1.697 ≈ 1.768. P(Z > 1.768) ≈ 0.0385. About 3.85%.' },
        { criterionId: 'c', maxPoints: 2, scoringCriteria: 'States the probability DECREASES; justifies by noting σ_x̄ at n = 200 is about half its value at n = 50, and reasons that reduced variability makes x̄ > 33 less likely (no full recomputation required).', modelResponse: 'DECREASE. With n = 200, σ_x̄ = 12/√200 ≈ 0.849 — about half the SD with n = 50. The sampling distribution is tighter around 30, so P(x̄ > 33) becomes much smaller (≈ 0.000196). Larger n → less variability → less likely to see extreme x̄.' },
      ] },
    ...
    { id: 'try-frq-twomeans', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 2.5, scoringCriteria: 'Computes μ_D = μ_A − μ_B = 30; computes σ_D by ADDING the two variances (σ²_A/36 + σ²_B/36) and taking the square root, ≈ 16.67 (not subtracting or averaging SDs directly).', modelResponse: 'μ_D = 750 − 720 = 30. σ²_D = 60²/36 + 80²/36 = 100 + 177.78 = 277.78. σ_D = √277.78 ≈ 16.67.' },
        { criterionId: 'b', maxPoints: 2, scoringCriteria: 'Standardizes D = 50 using μ_D and σ_D from part (a); computes P(Z > z) correctly.', modelResponse: 'Standardize: z = (50 − 30)/16.67 ≈ 1.20. P(Z > 1.20) ≈ 0.1151. About 11.5%.' },
        { criterionId: 'c', maxPoints: 2.5, scoringCriteria: 'Standardizes D = 0 and computes P(Z < z); correctly interprets this as P(x̄_A < x̄_B) — Brand A\'s sample mean falling below Brand B\'s despite Brand A\'s higher true mean, attributing this to sampling variability.', modelResponse: 'z = (0 − 30)/16.67 ≈ -1.80. P(Z < -1.80) ≈ 0.0359. About 3.6%. This is the probability that x̄_A < x̄_B — i.e., that in a randomly chosen pair of samples, Brand A\'s sample mean falls BELOW Brand B\'s sample mean (despite Brand A having higher true mean). Counter-intuitive but possible due to sampling variability.' },
      ] },
