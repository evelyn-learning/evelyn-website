...
export const APENGLANG_MOCK_A_PASSAGES: Passage[] = [
  ...
  },
  {
    ...
    fullText:
      ...
      `(11) Critics raise practical objections, and these deserve a fair hearing. (12) A later start can complicate bus schedules, after-school jobs, and sports practices, which often depend on the current timetable. (13) Parents who leave for work early may worry about leaving younger children unsupervised. (14) These concerns are real. (15) They are also, in most cases, solvable with planning, and several districts have already shown that the schedule can be rearranged without disaster.\n\n` +
      `(16) I understand that changing the start time is not as simple as moving the hands of a clock. (17) But we should be honest about what the current schedule costs. (18) Every morning, we ask students to learn at the exact hour their bodies are least prepared to, and then we wonder why so many of them struggle. (19) The problem is not the students. (20) A schedule built for the convenience of adults is being applied to the biology of teenagers, and the mismatch is doing real harm.\n\n` +
