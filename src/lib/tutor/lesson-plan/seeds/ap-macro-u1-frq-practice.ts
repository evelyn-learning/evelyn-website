...
export const SEED_AP_MACRO_U1_FRQ_PRACTICE: LessonPlan = {
  ...
  segments: [
    ...
    },
    {
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 2, scoringCriteria: 'Defines a production possibilities curve as a graph showing the maximum combinations of two goods an economy can produce given fixed resources and technology.', modelResponse: 'A production possibilities curve is a graph showing the maximum combinations of two goods an economy can produce given its fixed resources and technology.' },
        { criterionId: 'b', maxPoints: 3, scoringCriteria: 'Shows the slope computation for the linear PPC (80 cheese forgone / 40 wine gained) and states that the opportunity cost of 1 wine is 2 cheese.', modelResponse: 'The PPC is linear, so opportunity cost is constant: slope = 80 cheese / 40 wine = 2 cheese per wine. Producing 1 wine costs 2 cheese.' },
        { criterionId: 'c', maxPoints: 4, scoringCriteria: 'Names TWO valid policies that expand productive capacity (e.g., capital investment, education/human capital, immigration, technology investment, trade liberalization) and gives a brief mechanism for how each shifts the PPC outward — 2 points per policy: 1 for the policy, 1 for the mechanism.', modelResponse: '(i) INVEST IN CAPITAL — tax incentives for business investment or public infrastructure spending raise capital per worker, so the economy can produce more of both goods; the PPC shifts outward. (ii) IMPROVE EDUCATION / HUMAN CAPITAL — training and R&D subsidies make workers more productive, expanding output capacity for both goods; the PPC shifts outward.' },
      ] },
      responseFormat: 'frq',
      hints: [
    ...
    },
    {
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 2, scoringCriteria: 'Computes the opportunity cost of one phone in each country: 2 shirts in Country A (10/5) and 1 shirt in Country B (4/4) — 1 point each.', modelResponse: 'Country A: 1 phone costs 10/5 = 2 shirts. Country B: 1 phone costs 4/4 = 1 shirt.' },
        { criterionId: 'b', maxPoints: 3, scoringCriteria: 'Assigns comparative advantage in phones to Country B and in shirts to Country A, explaining each via the LOWER opportunity cost (1 shirt < 2 shirts for phones; 0.5 phones < 1 phone for shirts).', modelResponse: 'Country B has the comparative advantage in phones — its opportunity cost (1 shirt) is lower than A\'s (2 shirts). Country A has the comparative advantage in shirts — 1 shirt costs 0.5 phones in A versus 1 phone in B, so A\'s opportunity cost of shirts is lower.' },
        { criterionId: 'c', maxPoints: 1, scoringCriteria: 'States a terms-of-trade rate strictly between the two opportunity costs of a phone — between 1 and 2 shirts per phone.', modelResponse: 'Any rate strictly between 1 shirt per phone (B\'s opportunity cost) and 2 shirts per phone (A\'s opportunity cost) — for example, 1.5 shirts per phone — benefits both countries.' },
        { criterionId: 'd', maxPoints: 3, scoringCriteria: 'Evaluates the argument as WRONG by distinguishing absolute advantage (A produces more of everything) from comparative advantage (B has lower opportunity cost in phones) and stating that trade at a rate between the two opportunity costs lets both countries consume more than in autarky.', modelResponse: 'The president is WRONG. Country A has ABSOLUTE advantage in both goods (10>4 shirts, 5>4 phones per hour), but Country B has COMPARATIVE advantage in phones (lower opportunity cost). If A specializes in shirts and B in phones and they trade at any rate strictly between 1 and 2 shirts per phone, BOTH countries consume more than they could alone. Absolute advantage is irrelevant to the trade decision; comparative advantage determines mutually beneficial specialization.' },
      ] },
      responseFormat: 'frq',
      hints: [
    ...
    },
    {
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 2, scoringCriteria: 'Computes both MU/P schedules correctly: tacos (P=$3): 8, 6, 5, 3, 2; burritos (P=$6): 7, 5, 4, 2 — 1 point per schedule.', modelResponse: 'MU/P for tacos (P=$3): 1st=8, 2nd=6, 3rd=5, 4th=3, 5th=2. MU/P for burritos (P=$6): 1st=7, 2nd=5, 3rd=4, 4th=2.' },
        { criterionId: 'b', maxPoints: 3, scoringCriteria: 'Uses the highest-MU/P-first allocation method within the $24 budget and arrives at the utility-maximizing bundle of 4 tacos + 2 burritos (spending exactly $24, total utility 138 utils).', modelResponse: 'Buy in descending MU/P order: taco-1 (8), burrito-1 (7), taco-2 (6), burrito-2 (5), taco-3 (5), then taco-4 (3) since burrito-3 ($6) is unaffordable with $3 left. Final bundle: 4 tacos + 2 burritos = 4×$3 + 2×$6 = $24. Total utility = (24+18+15+9) + (42+30) = 138 utils.' },
        { criterionId: 'c', maxPoints: 2, scoringCriteria: 'States the equimarginal principle (MU/P equal across goods at the optimum) and notes it fails to hold exactly here — last taco MU/P = 3 vs last burrito MU/P = 5 — because of the integer purchase sizes and the binding budget.', modelResponse: 'Equimarginal principle: at the optimum, MU_taco/P_taco = MU_burrito/P_burrito. Here the last taco bought has MU/P = 3 while the last burrito has MU/P = 5 — NOT equal. The consumer would prefer burrito-3 (MU/P = 4) over taco-4 (MU/P = 3) but cannot afford a $6 burrito with $3 left. Strict equality fails because of the integer constraint and binding budget; with continuous units it would hold exactly.' },
        { criterionId: 'd', maxPoints: 2, scoringCriteria: 'Evaluates the friend as WRONG: the decision metric is MU PER DOLLAR, not raw MU; supports it with the comparison of all-burritos (4 burritos, 108 utils) versus the mixed bundle (138 utils) for the same $24.', modelResponse: 'The friend is WRONG. Higher raw MU does not mean better choice — what matters is MU PER DOLLAR, and burritos cost twice as much. All-burritos spends $24 on 4 burritos for 42+30+24+12 = 108 utils; the mixed bundle yields 138 utils — 30 more for the same money. The friend conflates raw MU with the dollar-adjusted decision metric.' },
      ] },
      responseFormat: 'frq',
      hints: [
