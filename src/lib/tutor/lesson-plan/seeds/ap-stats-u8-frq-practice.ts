...
export const SEED_AP_STATS_U8_FRQ_PRACTICE: LessonPlan = {
  ...
  segments: [
    ...
    { id: 'try-frq-gof', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'hypotheses', maxPoints: 1, scoringCriteria: 'States H_0: distribution is (red 0.474, black 0.474, green 0.052) and H_a: at least one proportion differs from the claimed value.', modelResponse: 'H_0: distribution is (red 0.474, black 0.474, green 0.052). H_a: at least one differs.' },
        { criterionId: 'expected-counts-conditions', maxPoints: 1, scoringCriteria: 'Computes expected counts as n·p_i (237, 237, 26), checks all E ≥ 5, and verifies conditions (random spins/independence, Large Counts).', modelResponse: 'Expected: 500(0.474) = 237; 237; 26. All ≥ 5 ✓. Conditions: Random spins (assume independence) ✓; Large Counts ✓.' },
        { criterionId: 'mechanics', maxPoints: 3.5, scoringCriteria: 'Correctly computes χ² = Σ(O−E)²/E ≈ 2.305, uses df = k − 1 = 2, and finds p-value ≈ 0.316 from χ²_2.', modelResponse: 'χ² = (230−237)²/237 + (250−237)²/237 + (20−26)²/26 = 49/237 + 169/237 + 36/26 = 0.2068 + 0.7131 + 1.3846 ≈ 2.305. df = 3 − 1 = 2. p-value = P(χ²_2 ≥ 2.305) ≈ 0.316.' },
        { criterionId: 'conclusion', maxPoints: 2.5, scoringCriteria: 'Compares p-value to α (0.316 > 0.05), makes the correct decision (fail to reject H_0), and states the conclusion in context of the casino\'s claimed distribution.', modelResponse: '0.316 > 0.05 → fail to reject H_0. Conclusion: "Because p ≈ 0.32 > α = 0.05, we fail to reject H_0. There is NOT convincing statistical evidence that the wheel\'s output deviates from the casino\'s claimed distribution."' },
      ] },
    ...
    { id: 'try-frq-twoway', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1.5, scoringCriteria: 'Identifies HOMOGENEITY as the appropriate test and justifies with three SEPARATE random samples (one per city) — multiple populations, not one sample split by two variables.', modelResponse: 'HOMOGENEITY — three SEPARATE random samples (one from each city) — multiple populations.' },
        { criterionId: 'b', maxPoints: 1.5, scoringCriteria: 'States H_0: the distribution of commute mode is the SAME across the three cities, and H_a: the distributions DIFFER.', modelResponse: 'H_0: distribution of commute mode is the SAME across the three cities. H_a: distributions DIFFER.' },
        { criterionId: 'c', maxPoints: 6, scoringCriteria: 'Computes correct expected counts via E = (row·col)/grand total (25, 16.67, 58.33 per city), checks all E ≥ 5, computes χ² = Σ(O−E)²/E ≈ 25.573 with df = (r−1)(c−1) = 4, finds p-value ≈ 0.00004, and concludes (reject H_0) in context comparing p to α = 0.05.', modelResponse: 'Column totals: walk = 75; bike = 50; drive = 175. Grand total: 300. Expected (using row total = 100 for each city): E_walk = 100(75)/300 = 25 (each city). E_bike = 100(50)/300 ≈ 16.67. E_drive = 100(175)/300 ≈ 58.33. All ≥ 5 ✓. χ² = (25−25)²/25 + (15−16.67)²/16.67 + (60−58.33)²/58.33 + (35−25)²/25 + (25−16.67)²/16.67 + (40−58.33)²/58.33 + (15−25)²/25 + (10−16.67)²/16.67 + (75−58.33)²/58.33 = 0 + 0.167 + 0.048 + 4 + 4.165 + 5.762 + 4 + 2.667 + 4.764 = 25.573. df = (3−1)(3−1) = 4. p-value = P(χ²_4 ≥ 25.573) ≈ 0.000040. 0.000040 < 0.05 → REJECT H_0. Conclusion: "Because p < 0.001 < α = 0.05, we reject H_0. There IS convincing statistical evidence that commute-mode distributions DIFFER across the three cities."' },
      ] },
