...
export const SEED_AP_APWORLD_U2_SILK_ROADS: LessonPlan = {
  ...
  segments: [
    ...
    },
    {
      ...
      kind: 'worked_example',
      problem:
        'Analyze this excerpt from Marco Polo\'s account of the city of Kinsay (Hangzhou), which he visited during the reign of Kublai Khan: "The city is beyond dispute the finest and the noblest in the world. The number and wealth of the merchants, and the amount of goods that passed through their hands, were so enormous that no man could form a just estimate thereof. The market-places were thronged and crammed with purchasers, and with the traders who had brought in stores of provisions by land or water; and all was kept in order." What does this passage reveal about the scale of Silk-Road-era commerce, and what should a careful reader keep in mind about its reliability?',
      steps: [
        ...
        'IDENTIFY THE CLAIM. Polo asserts that Kinsay\'s commerce was so vast — "the number and wealth of the merchants... so enormous" — that its scale defied estimation, and that goods arrived continuously "by land or water" from multiple directions.',
        "CONNECT TO THE COMMERCIAL-VOLUME CAUSE. This is direct testimony for the concept's claim that trading cities grew wealthy as network nodes: a single Yuan-era city hosting more commerce than a European visitor could even quantify shows how much wealth and goods a secured, credit-enabled trade network could funnel into one hub.",
        'CONNECT TO THE POLITICAL-SECURITY CAUSE. Polo notes the markets were "kept in order" — a small but telling detail. Order in a crowded, wealthy marketplace reflects the kind of stable governance the concept links to Pax Mongolica: without political security, a market this valuable would be a magnet for disorder and predation, not calm commerce.',
        'WEIGH THE SOURCE\'S RELIABILITY. Polo is an outside observer with an incentive to impress his European readers, using superlatives ("finest and noblest in the world," "so enormous that no man could form a just estimate") that historians treat as descriptive color rather than precise measurement. His account is still valuable as evidence THAT large-scale, orderly commerce existed at Kinsay — just not as a literal statistic.',
      ...
      answer:
        "Polo's account of Kinsay shows a city whose commerce was, in his telling, too vast to estimate — merchants, goods, and buyers converging \"by land or water\" into markets that were nonetheless \"kept in order.\" That combination is exactly what the concept predicts: a trading city grown wealthy as a node in a network revived by commercial tools like credit and paper money, operating with a level of order consistent with the political stability of Pax Mongolica. A careful reader should note Polo is a foreign merchant writing for an audience back in Venice, prone to superlative language (\"finest and noblest in the world\") that overstates precision even while it reliably signals that Kinsay's commerce genuinely impressed an experienced outside observer.",
      estimatedMinutes: 5,
    },
    {
      id: 'try-saq',
