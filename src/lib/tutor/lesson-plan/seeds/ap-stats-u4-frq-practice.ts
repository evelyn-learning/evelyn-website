...
export const SEED_AP_STATS_U4_FRQ_PRACTICE: LessonPlan = {
  ...
  segments: [
    ...
    { id: 'try-frq-conditional', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1.5, scoringCriteria: 'Correct conditional probability formula P(obese | exercise) = P(both)/P(exercise), and correct value 0.05/0.30 = 1/6 ≈ 0.167.', modelResponse: 'P(obese | exercise) = P(both)/P(exercise) = 0.05/0.30 = 1/6 ≈ 0.167.' },
        { criterionId: 'b', maxPoints: 1.5, scoringCriteria: 'Correct conditional probability formula P(exercise | obese) = P(both)/P(obese), and correct value 0.05/0.25 = 0.20.', modelResponse: 'P(exercise | obese) = P(both)/P(obese) = 0.05/0.25 = 0.20.' },
        { criterionId: 'c', maxPoints: 3, scoringCriteria: 'Correct independence test comparing P(exercise)·P(obese) to P(both) (or equivalently comparing a conditional probability to the marginal), correct conclusion of NOT independent, and a justification referencing the direction/size of the discrepancy.', modelResponse: 'Test independence: P(exercise) · P(obese) = 0.30 · 0.25 = 0.075. P(both) = 0.05 ≠ 0.075. NOT independent. (Equivalently, P(obese | exercise) = 0.167 ≠ P(obese) = 0.25 — these aren\'t equal.) Specifically: obesity is LESS LIKELY among those who exercise (0.167 < 0.25), suggesting a NEGATIVE association.' },
      ] },
    ...
    { id: 'try-frq-binomial', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1.5, scoringCriteria: 'Checks all four BINS conditions by name (Binary, Independent, n fixed, Same p) and states X ~ Binomial(12, 0.20) with correct n and p.', modelResponse: 'BINS: Binary (right/wrong); Independent (different questions, no carryover); n = 12 fixed; Same p = 0.20 (1 of 5 correct by guess). X ~ Binomial(12, 0.20).' },
        { criterionId: 'b', maxPoints: 1.5, scoringCriteria: 'Correct formulas μ = np and σ = √(np(1−p)) with correct substitution, yielding μ = 2.4 and σ ≈ 1.386.', modelResponse: 'μ = np = 12(0.20) = 2.4. σ = √(np(1−p)) = √(12·0.20·0.80) = √1.92 ≈ 1.386.' },
        { criterionId: 'c', maxPoints: 2, scoringCriteria: 'Correctly sets up P(X ≥ 5) as the complement 1 − P(X ≤ 4), uses binomcdf(12, 0.20, 4) correctly, and reports P(X ≥ 5) ≈ 0.0726.', modelResponse: 'P(X ≥ 5) = 1 − P(X ≤ 4) = 1 − binomcdf(12, 0.20, 4). binomcdf(12, 0.20, 4) ≈ 0.9274. P(X ≥ 5) ≈ 0.0726.' },
        { criterionId: 'd', maxPoints: 2, scoringCriteria: 'Correctly computes P(X ≥ 8) = 1 − binomcdf(12, 0.20, 7) ≈ 0.0006 and gives a contextual comment that passing by guessing alone is extremely unlikely.', modelResponse: 'P(X ≥ 8) = 1 − binomcdf(12, 0.20, 7) ≈ 1 − 0.9994 ≈ 0.0006. The chance of passing by guessing alone is about 0.06% — vanishingly small.' },
      ] },
    ...
    { id: 'try-frq-combine', kind: 'try_yourself',
      ...
      rubric: { parts: [
        { criterionId: 'a', maxPoints: 1.5, scoringCriteria: 'Correctly expresses R as the linear combination R = 3.50·X + 4.50·Y, with correct coefficients matching the per-drink revenue.', modelResponse: 'R = 3.50·X + 4.50·Y (each small sale brings in $3.50; each medium $4.50).' },
        { criterionId: 'b', maxPoints: 2, scoringCriteria: 'Correct formula μ_R = 3.50·μ_X + 4.50·μ_Y with correct substitution, yielding μ_R = $550.', modelResponse: 'μ_R = 3.50·μ_X + 4.50·μ_Y = 3.50(80) + 4.50(60) = 280 + 270 = $550.' },
        { criterionId: 'c', maxPoints: 3.5, scoringCriteria: 'States that variances add under independence (with each coefficient squared), correctly computes σ²_R = (3.50)²·σ²_X + (4.50)²·σ²_Y = 3789, and takes the square root to get σ_R ≈ $61.56.', modelResponse: 'σ²_R = (3.50)²·σ²_X + (4.50)²·σ²_Y (independence; variances add after scaling by coefficient²) = (12.25)(144) + (20.25)(100) = 1764 + 2025 = 3789. σ_R = √3789 ≈ $61.56.' },
      ] },
