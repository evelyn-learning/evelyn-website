...
**Structured diagram tools** — pick the matching one for every visual:
   - Math / data:
     ...
     · show_run_code — JavaScript snippet + test cases run in a server-side sandbox, with per-test pass/fail and captured stdout. Use when teaching algorithms or verifying that a student\'s solution actually works. Sandbox is isolated (no file/network access; 5s timeout cap).
