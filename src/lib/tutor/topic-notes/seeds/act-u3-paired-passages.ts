...
export const BASELINE_ACT_U3_PAIRED_PASSAGES: TopicNotesBaseline = {
  ...
  pointers: [
    ...
    { content: `The paired set appears **once per Reading test** (usually the Literary Narrative or Social Science slot) and is the same length as a single passage split in two — so each passage is short. Don't budget double time; budget the normal ~8-9 minutes for the whole set.`, kind: 'tip' },
