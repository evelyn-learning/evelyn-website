...
export const BASELINE_ACT_U4_SCIENCE_PACING_STRATEGY: TopicNotesBaseline = {
  ...
  theory: [
    { loId: 'act.science-pacing-strategy', content: `SCIENCE IS OPTIONAL: unlike English, Math, and Reading, the Science section is an add-on — it doesn't count toward your Composite score. Students who opt in get a separate Science score plus a STEM score (the average of Math and Science). Everything below is for students who ARE taking it, where the clock is still very real.` },
    { loId: 'act.science-pacing-strategy', content: `THE MATH: 40 minutes across 6-7 passages averages ~5-6 minutes each, but the three passage types are NOT equally fast — budget them differently, not evenly.` },
    { loId: 'act.science-pacing-strategy', content: `TRIAGE ORDER: Data Representation is fastest (~4 min — pure lookup, no experiment to track) and goes FIRST. Research Summaries are next (~5 min — comparing 2-3 experiments). Conflicting Viewpoints is slowest (~7-8 min — the only type that requires reading full passages of text) and goes LAST, regardless of where it is printed in the booklet.` },
  ...
  methods: [
    {
      ...
      steps: [
        ...
        `Apply the standard per-type budgets: Data Rep ≈ 4 min each, Research Summary ≈ 5 min each, Conflicting Viewpoints ≈ 8 min.`,
        `Multiply out each type: 3 Data Rep × 4 min = 12 min. 3 Research Summary × 5 min = 15 min. 1 Conflicting Viewpoints × 8 min = 8 min.`,
        'Sum the working time: 12 + 15 + 8 = 35 minutes.',
        `Add the initial 1-minute scan to tag passage types: 35 + 1 = 36 minutes, leaving about 4 minutes of buffer in the 40-minute section for whichever passage runs long.`,
      ...
      example: { problem: `Today's ACT Science section has 7 passages: three Data Representation, three Research Summaries, and one Conflicting Viewpoints. Using the standard per-type budgets, build a time plan for the full 40-minute section and decide the order to tackle the passages.`, solution: `Work the 3 Data Rep passages first (~4 min each = 12 min), then the 3 Research Summaries (~5 min each = 15 min), then Conflicting Viewpoints last (~8 min) — 35 working minutes plus the 1-minute scan leaves about 4 minutes of buffer in the 40-minute section.` },
  ...
  pointers: [
    ...
    { content: `Triage by passage type: Data Representation first (~4 min, fastest), Research Summaries second (~5 min), Conflicting Viewpoints last (~7-8 min, only type needing full-text reading).`, kind: 'tip' },
