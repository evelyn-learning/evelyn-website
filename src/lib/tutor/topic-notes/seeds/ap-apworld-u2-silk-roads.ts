...
export const BASELINE_AP_APWORLD_SILK_ROADS: TopicNotesBaseline = {
  ...
  theory: [
    ...
    },
    {
      ...
      content:
        'Marco Polo, a Venetian merchant residing in Mongol-ruled China in the late 1200s, described Kinsay (Hangzhou) as having commerce "so enormous that no man could form a just estimate thereof," with markets that were nonetheless "kept in order." As an outside observer writing for a European audience, his superlatives are descriptive color rather than a precise measurement — but his account still shows a trading city doing business at a scale that awed an experienced foreign merchant.',
    },
    {
      loId: 'apworld.silk-roads',
      kind: 'trap',
  ...
  methods: [
    {
      ...
      example: {
        problem:
          'Analyze this excerpt from Marco Polo\'s account of Kinsay (Hangzhou): "The city is beyond dispute the finest and the noblest in the world. The number and wealth of the merchants, and the amount of goods that passed through their hands, were so enormous that no man could form a just estimate thereof... and all was kept in order." What does this reveal about the scale of Silk-Road-era commerce, and what should a careful reader keep in mind about its reliability?',
        solution:
          'Historical context: Polo visited Kinsay while residing in Yuan China under Kublai Khan in the late 1200s, later dictating an account (c. 1300) for a European audience unfamiliar with Asia. Purpose: to impress readers with the scale of Chinese commerce. Point of view: as an outside merchant with an incentive to awe his readers, his superlatives ("finest and noblest," "so enormous") should be read as descriptive color, not a literal statistic. Connecting to cause: "kept in order" is a telling detail linking the passage to Pax Mongolica — a market this valuable stayed orderly because of the political stability the Mongols secured, while the sheer described volume of goods and merchants is direct evidence for the concept\'s claim that trading cities grew wealthy as network nodes.',
      },
