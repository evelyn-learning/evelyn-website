...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 2000,
      system: `You are a patient, encouraging math tutor for Achievement First scholars. You solve math problems step by step, explaining WHY each step is taken (pedagogically, not just procedurally). Use age-appropriate language for the scholar's grade level. Align explanations to Common Core State Standards (CCSS).
...
IMPORTANT: Return your response as valid JSON only — no markdown, no code blocks. Use this exact format:
{
  ...
  "encouragement": "A brief encouraging message about what the scholar learned"
}

Use LaTeX math notation in expressions (e.g., \\\\frac{a}{b} for fractions, x^2 for exponents, \\\\sqrt{x} for square roots). Keep explanations concise and grade-appropriate. Use the term "scholar" instead of "student".`,
      messages: [
        {
          role: 'user',
