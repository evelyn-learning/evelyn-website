...
// ─── Topic data (BC Curriculum) ──────────────────────────────────────
const TOPICS: Record<string, Record<string, string[]>> = {
  Math: {
    'Grade 3': ['Addition & Subtraction', 'Multiplication Basics', 'Fractions Introduction', 'Patterns & Sorting'],
    'Grade 4': ['Multiplication & Division', 'Fractions', 'Decimals', 'Geometry Basics'],
    'Grade 5': ['Fractions', 'Decimals', 'Geometry', 'Patterns & Algebra'],
    'Grade 6': ['Ratios', 'Integers', 'Expressions & Equations', 'Geometry & Measurement'],
    'Grade 7': ['Proportional Reasoning', 'Integers & Operations', 'Geometry', 'Data Analysis'],
    'Grade 8': ['Linear Equations', 'Pythagorean Theorem', 'Surface Area & Volume', 'Probability'],
    'Grade 9': ['Linear Relations', 'Polynomials', 'Statistics', 'Financial Literacy'],
    'Grade 10': ['Quadratic Functions', 'Trigonometry', 'Systems of Equations', 'Measurement'],
  },
  Science: {
    'Grade 3': ['Plant & Animal Life Cycles', 'Forces & Motion', 'Weather & Water'],
    'Grade 4': ['Habitats & Ecosystems', 'Light & Sound', "Earth's Surface"],
    'Grade 5': ['Human Body Systems', 'Properties of Matter', 'Earth & Space'],
    'Grade 6': ['Cells & Organisms', 'Electricity', 'Space & Astronomy'],
    'Grade 7': ['Life Science (Organisms)', 'Earth Science', 'Physical Science (Chemistry)'],
    'Grade 8': ['Cells & Genetics', 'Plate Tectonics', 'Physics (Forces & Motion)'],
    'Grade 9': ['Atoms & Elements', 'Reproduction', 'Ecosystems'],
    'Grade 10': ['Chemical Reactions', 'Motion & Forces', 'Energy Transformations'],
  },
};
...
export default function PracticeTestGenerator() {
  ...
  const availableTopics = TOPICS[subject]?.[grade] || [];
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-purple-100 overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-purple-50 bg-gradient-to-r from-purple-50 to-white">
        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <span className="text-purple-600">2.</span> Practice Test Generator
        </h2>
        <p className="text-gray-600 mt-1">
          Unlimited, unique practice tests for any topic and grade level
        </p>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-end">
          ...
          >
            ...
            ) : questions.length > 0 ? 'Generate Another Test' : 'Generate Test'}
          </button>
        </div>
        ...
        {testsGenerated > 0 && (
          <div className="flex items-center gap-2 text-sm text-purple-600 bg-purple-50 rounded-lg px-4 py-2">
            ...
            Every test is unique — AI-generated and curriculum-aligned
            {testsGenerated > 1 && <span className="font-semibold ml-1">({testsGenerated} tests generated)</span>}
          </div>
        )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                    {q.options.map((opt, oi) => {
                      ...
                      if (isSelected && !submitted) {
                        optionClass = 'border-purple-500 bg-purple-50 ring-2 ring-purple-200';
                      }
                      if (submitted && isRight) {
                        optionClass = 'border-green-500 bg-green-50';
                      }
                      if (submitted && isSelected && !isRight) {
                ...
                <div className={`inline-flex items-center gap-3 px-8 py-4 rounded-2xl ${
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-gray-500">
                      {score === 5 ? 'You\'ve mastered this topic!' : 'Review the explanations above for incorrect answers.'}
                    </p>
                  </div>
                </div>
                <div className="mt-4">
        ...
        {!loading && questions.length === 0 && !error && (
          <div className="text-center py-12 text-gray-400">
            ...
            <p>Select a grade, subject, and topic, then click &quot;Generate Test&quot;</p>
          </div>
        )}
      </div>
