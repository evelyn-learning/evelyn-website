...
import { getLessonById, LessonSegment } from '../../data/lessons';
import VideoEmbed from '../shared/VideoEmbed';

function NarrationCard({ segment }: { segment: Extract<LessonSegment, { type: 'narration' }> }) {
  ...
  return (
    <div className="border-l-4 border-[#003B71] bg-white rounded-xl p-6 shadow-sm">
      <div className="flex items-start gap-4">
        <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
          <svg className="w-5 h-5 text-[#003B71]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="flex-1">
          ...
          <button
            onClick={readAloud}
            className="mt-3 text-xs px-3 py-1.5 rounded-lg bg-blue-50 text-[#003B71] hover:bg-blue-100 transition-colors flex items-center gap-1.5"
          >
...
function VideoCard({ segment }: { segment: Extract<LessonSegment, { type: 'video' }> }) {
  return (
...
function QuestionCard({
  ...
  return (
    <div className={`bg-white rounded-xl p-6 shadow-sm border-2 ${
      ...
      <p className="text-xs font-semibold text-[#003B71] uppercase tracking-wider mb-1">{label}</p>
      <p className="font-semibold text-gray-800 mb-4">{segment.question}</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
        {segment.options.map((opt, i) => {
          let cls = 'border-gray-200 hover:border-[#003B71]/30 cursor-pointer';
          if (selected === i && !submitted) cls = 'border-[#003B71] bg-blue-50 ring-2 ring-blue-200';
      ...
      {!submitted ? (
        <button
          ...
          className="px-5 py-2 rounded-lg bg-[#003B71] text-white text-sm font-medium hover:bg-[#002a55] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          Check Answer
        </button>
      ) : (
...
function SummaryCard({
  ...
  return (
    <div className="bg-white rounded-xl p-8 shadow-sm text-center space-y-6 border-2 border-green-200">
      ...
      <button
        onClick={onFinish}
        className="px-8 py-3 rounded-xl bg-[#003B71] hover:bg-[#002a55] text-white font-semibold transition-colors"
      >
        Back to Home
      </button>
    </div>
...
export default function LessonPlayer() {
  const { activeLessonId, lessonSegmentIndex, advanceSegment, completeLesson, navigate } = useAFStore();
  ...
  if (!lesson) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-12 text-center">
        ...
        <button onClick={() => navigate('student-home')} className="mt-4 text-[#003B71] hover:underline">
          Back to Home
        </button>
      </div>
    );
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <button
          ...
          className="text-sm text-gray-500 hover:text-[#003B71] transition-colors flex items-center gap-1"
        >
      ...
      <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
        <div
          className="h-full bg-[#003B71] rounded-full transition-all duration-300"
          style={{ width: `${((currentIndex + 1) / segments.length) * 100}%` }}
        />
      </div>

      <div className="min-h-[300px]">
        {currentSegment?.type === 'narration' && <NarrationCard segment={currentSegment} />}
        {currentSegment?.type === 'video' && <VideoCard segment={currentSegment} />}
      ...
      {currentSegment?.type !== 'summary' && (
        <div className="flex justify-center">
          <button
            ...
            className="px-8 py-3 rounded-xl bg-[#F5A623] hover:bg-[#e6991a] text-[#003B71] font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
          >
