...
const SAMPLE_PROBLEMS_3: SampleProblem[] = [
  {
    ...
    latex: '\\frac{1}{4} + \\frac{2}{4}',
    solution: {
      steps: [
        { step: 1, title: 'Check the denominators', expression: '\\frac{1}{4} + \\frac{2}{4} \\quad \\text{(same denominator: 4)}', explanation: 'Both fractions have the same denominator (4), so we can add them directly!' },
        { step: 2, title: 'Add the numerators', expression: '\\frac{1 + 2}{4} = \\frac{3}{4}', explanation: 'Keep the denominator the same and add the top numbers: 1 + 2 = 3.' },
      ],
      finalAnswer: '\\frac{3}{4}',
      encouragement: 'Great job! When fractions have the same denominator, just add the numerators!',
    },
  },
  {
    grade: 'Grade 3',
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      <button
        ...
        className="text-sm text-gray-500 hover:text-[#003B71] transition-colors flex items-center gap-1 mb-6"
      >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-blue-50 bg-gradient-to-r from-blue-50 to-white">
          <h2 className="text-xl font-bold text-gray-900">Math Problem Solver</h2>
          <p className="text-gray-600 text-sm mt-1">Step-by-step solutions — like having a personal math tutor</p>
        </div>
        ...
        <div className="p-6">
          {mode === 'select' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Try a Sample Problem</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {samples.map((p) => (
                    <button
                      key={p.title}
                      onClick={() => selectSample(p)}
                      className="text-left p-4 rounded-xl border-2 border-blue-100 hover:border-[#003B71]/40 hover:shadow-md transition-all group"
                    >
                      <span className="text-xs font-semibold text-[#003B71]">{p.grade}</span>
                      <p className="font-semibold text-gray-800 mt-1 group-hover:text-[#003B71] transition-colors">{p.title}</p>
              ...
              <div className="border-t border-gray-100 pt-6">
                <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Or Type Your Own Problem</h3>
                <form onSubmit={handleCustomSubmit} className="flex flex-col sm:flex-row gap-3">
                  <input
                    type="text"
                    value={customProblem}
                    onChange={(e) => setCustomProblem(e.target.value)}
                    placeholder="e.g., What is 3/8 + 2/8?"
                    className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#003B71] focus:border-transparent"
                  />
                  <button
                    type="submit"
                    disabled={!customProblem.trim()}
                    className="px-6 py-2.5 rounded-xl bg-[#003B71] hover:bg-[#002a55] text-white font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Solve It
                  </button>
                </form>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="bg-blue-50 rounded-xl px-5 py-3">
                  <span className="text-xs font-semibold text-[#003B71] block mb-1">
                ...
                <button onClick={reset} className="text-sm text-gray-500 hover:text-[#003B71] transition-colors flex items-center gap-1">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                  </svg>
                  Try another problem
              ...
              {loading && (
                <div className="flex items-center justify-center py-12">
                  <div className="text-center">
                    <div className="animate-spin rounded-full h-10 w-10 border-4 border-blue-200 border-t-[#003B71] mx-auto mb-3" />
                  ...
                  <div className="bg-gray-50 rounded-xl p-6 border border-gray-100 space-y-4">
                    {solution.steps.map((step, i) => (
                      ...
                      >
                        ...
                        {i < visibleSteps - 1 && <div className="ml-4 h-6 border-l-2 border-blue-200" />}
                      </div>
                    ))}
                    ...
                    {visibleSteps < solution.steps.length ? (
                      <>
                        <button
                          ...
                          className="px-6 py-2.5 rounded-xl bg-[#F5A623] hover:bg-[#e6991a] text-[#003B71] font-semibold transition-colors flex items-center gap-2"
                        >
                      ...
                      <button
                        onClick={reset}
                        className="px-6 py-2.5 rounded-xl bg-[#003B71] hover:bg-[#002a55] text-white font-semibold transition-colors"
                      >
