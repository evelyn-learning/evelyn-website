...
export default function PracticeTest() {
  ...
  const generateTest = async () => {
    ...
    try {
      const res = await fetch('/api/showcase/achievement-first/practice-test', {
        method: 'POST',
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-blue-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-blue-50 bg-gradient-to-r from-blue-50 to-white">
          ...
          <p className="text-gray-600 text-sm mt-1">Generate unique practice tests for {gradeLabel} topics</p>
        </div>
        ...
        <div className="p-6 space-y-6">
          <div className="flex flex-col sm:flex-row gap-3 items-end">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-[#003B71] hover:bg-[#002a55] text-white font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
            >
              {loading ? (
          ...
          {testsGenerated > 0 && (
            <div className="flex items-center gap-2 text-sm text-[#003B71] bg-blue-50 rounded-lg px-4 py-2">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
              Every test is unique and CCSS-aligned
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <p className="font-semibold text-gray-800">
                        <span className="text-[#003B71] mr-1">{qi + 1}.</span>{' '}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                      {q.options.map((opt, oi) => {
                        ...
                        let optionClass = 'border-gray-200 hover:border-[#003B71]/30 hover:bg-blue-50 cursor-pointer';
                        if (isSelected && !submitted) optionClass = 'border-[#003B71] bg-blue-50 ring-2 ring-blue-200';
              ...
              {!submitted ? (
                <div className="text-center">
                  <button
                    ...
                    className="px-8 py-3 rounded-xl bg-[#F5A623] hover:bg-[#e6991a] text-[#003B71] font-semibold text-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Submit Answers
                  </button>
                  {!allAnswered && (
                ...
                <div className="text-center py-4">
                  ...
                  <div className="mt-4">
                    <button
                      onClick={generateTest}
                      className="px-6 py-2.5 rounded-xl bg-[#003B71] hover:bg-[#002a55] text-white font-medium transition-colors"
                    >
