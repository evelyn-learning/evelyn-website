...
export default function StudentHome() {
  const { currentUser, startLesson, navigate, completedLessons, assignedLessons, assignedTests, setPreSelectedTestTopic } = useAFStore();
  ...
  const instructorName = currentUser.grade === 3 ? 'Ms. Williams' : 'Mr. Jackson';
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-16 h-16">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
              <circle cx="18" cy="18" r="15.5" fill="none" stroke="#bfdbfe" strokeWidth="3" />
              <circle
                cx="18" cy="18" r="15.5" fill="none" stroke="#003B71" strokeWidth="3"
                strokeDasharray={`${overallProgress} ${100 - overallProgress}`}
                strokeLinecap="round"
              />
            </svg>
            <span className="absolute inset-0 flex items-center justify-center text-sm font-bold text-[#003B71]">
              {overallProgress}%
            </span>
          </div>
      ...
      {nextLesson && (
        <button
          ...
          className="w-full flex items-center gap-4 p-5 rounded-2xl bg-gradient-to-r from-[#003B71] to-[#00295a] text-white hover:from-[#002a55] hover:to-[#001a3d] transition-all shadow-lg group"
        >
          <div className="w-14 h-14 rounded-xl bg-white/20 flex items-center justify-center text-2xl flex-shrink-0">
            {nextLesson.icon}
          </div>
          <div className="flex-1 text-left">
            <p className="text-sm text-blue-200">Continue Learning</p>
            <p className="text-lg font-bold">{nextLesson.title}</p>
            <p className="text-sm text-blue-200 mt-0.5">{nextLesson.subject} &middot; Grade {nextLesson.grade}</p>
          </div>
            ...
            return (
              <button
                ...
                className={`text-left p-5 rounded-xl border-2 transition-all group ${
                    ...
                    : 'border-gray-100 bg-white hover:border-[#003B71]/30 hover:shadow-md'
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-gray-800 group-hover:text-[#003B71] transition-colors">
                        {lesson.title}
                      </p>
                      {done && (
                    ...
                    <span className="inline-block text-xs px-2 py-0.5 rounded-full mt-2 bg-blue-100 text-blue-700 font-medium">
                      {lesson.subject}
                    </span>
                  </div>
                </div>
              ...
              >
                <div className="flex items-start gap-3">
                  <span className="text-2xl">
                    <svg className="w-7 h-7 text-[#F5A623]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-1">Take a practice test to check your understanding.</p>
                    <div className="flex items-center gap-2 mt-2">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <button
              ...
              className="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-gray-100 bg-white hover:border-[#003B71]/30 hover:shadow-md transition-all group"
            >
              <div className="text-gray-400 group-hover:text-[#003B71] transition-colors">
                {tool.icon}
              </div>
              <span className="text-sm font-medium text-gray-700 group-hover:text-[#003B71] transition-colors text-center">
                {tool.label}
              </span>
            </button>
          ))}
