...
export const WHITEBOARD_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'Plot mathematical functions, curves, points, and shaded regions on a coordinate plane. LaTeX-style expressions for `expr` (use `functions` for y=f(x), `functionsOfY` for x=f(y)).',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string' },
  ...
  },
  {
    ...
    description: 'PREFER THIS over show_geometry whenever the figure can be described as a construction (chord on a circle, tangent at a point, perpendicular bisector, regular polygon, triangle center, intersection of two circles, incircle, circumcircle, etc). You declare GIVENS (raw points/circles/segments/lines/polygons) and STEPS (constructions referenced by id). Steps may also include explicit segment/line/polygon/circle declarations — the brain often prefers to compose top-to-bottom (declare points → connect them → derive things). The renderer solves the geometry exactly — you never compute coordinates for derived points and never get them wrong. Step kinds and fields: midpoint{ of }; point_on_circle{ on, angle }; chord{ on, length, direction, position, endpoints? }; radius{ on, to }; diameter{ on, direction|through, endpoints? }; tangent_at{ on, point, length? }; tangent_from{ on, external, prefer?, touchId? }; perpendicular_bisector{ of }; perpendicular_from{ from, to, footId? }; parallel_through{ through, of, length? }; intersect{ of: [a,b], prefer?, secondId? }; polygon_regular{ on, sides, rotation?, vertexIds? }; triangle_center{ vertices: [a,b,c], type }; segment{ from, to }; line{ through: [a,b] }; polygon{ vertices }; circle{ center, radius }; circle_through_point{ center, through }; circle_through_three{ points: [a,b,c] }; incircle{ vertices: [a,b,c], centerId?, tangentIds? }; circumcircle{ vertices: [a,b,c], centerId? }; ray{ from, toward, length?, endId? }; angle_bisector{ vertex, from, to, length? }; external_angle_bisector{ vertex, from, to, length? }; section_point{ of, ratio: [m,n] }; reflect_point{ point, across } — across accepts an id, "x-axis"/"y-axis" keywords, or inline { through: [a, b] } where a/b are point ids OR {x,y} literals (same flexibility for perpendicular_from.to and parallel_through.of); rotate_point{ point, around, angle }; translate_point{ point, by: {dx,dy} }; dilate_point{ point, about, factor }; excircle{ vertices, opposite?: "first"|"second"|"third", centerId?, tangentIds? }; tangents_from_external{ on, external, segmentIds?, touchIds? }; arc{ on, from, to, direction?: "ccw"|"cw" }; sector{ on, from, to, direction?, arcSegments? }. Multi-output steps name auto-generated children with UNDERSCORE: chord id "ch" → "ch_from"/"ch_to"; polygon_regular id "hex" → "hex_v0"…"hex_v5"; incircle id "inc" → "inc_center", "inc_T1", "inc_T2", "inc_T3"; circumcircle id "cc" → "cc_center". For a specific name pass endpoints/vertexIds/touchId/footId/centerId/tangentIds directly. Use display.showCoords / showLength / labels / colors to control text. Reserve show_geometry for free-form sketches that don\'t fit a construction language.',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string' },
  ...
  {
    name: 'show_stats',
    description: 'Statistical charts: histogram, boxplot, dotplot, bar, pie, or distribution curve. For `type: "distribution"`, provide `distribution.family`, `distribution.params`, and `distribution.shade` for a shaded probability region; `distribution.probabilityLabel` writes the computed probability inside the shaded region.',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string' },
