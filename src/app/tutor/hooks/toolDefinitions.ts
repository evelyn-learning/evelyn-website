...
export const WHITEBOARD_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'PREFER THIS over show_geometry whenever the figure can be described as a construction (chord on a circle, tangent at a point, perpendicular bisector, regular polygon, triangle center, intersection of two circles, etc). You declare GIVENS (raw points/circles/segments/lines/polygons) and STEPS (constructions referenced by id). The renderer solves the geometry exactly — you never compute coordinates for derived points and never get them wrong. Each step has one closed-form solution. Step kinds and field names: midpoint{ of }; point_on_circle{ on, angle }; chord{ on, length, direction, position, endpoints? }; radius{ on, to }; diameter{ on, direction|through, endpoints? }; tangent_at{ on, point, length? }; tangent_from{ on, external, prefer?, touchId? }; perpendicular_bisector{ of }; perpendicular_from{ from, to, footId? }; parallel_through{ through, of, length? }; intersect{ of: [a,b], prefer?, secondId? }; polygon_regular{ on, sides, rotation?, vertexIds? }; triangle_center{ vertices: [a,b,c], type }. Multi-output steps name auto-generated children with UNDERSCORE: chord id "ch" produces "ch_from" / "ch_to"; polygon_regular id "hex" produces "hex_v0"…"hex_v5". For a specific name pass endpoints/vertexIds/touchId/footId directly. Use display.showCoords / showLength / labels / colors to control text. Reserve show_geometry for free-form sketches that don\'t fit a construction language.',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string' },
