...
export const WHITEBOARD_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'PREFER THIS over show_geometry whenever the figure can be described as a construction (chord on a circle, tangent at a point, perpendicular bisector, regular polygon, triangle center, intersection of two circles, etc). You declare GIVENS (raw points/circles/segments/lines/polygons) and STEPS (constructions referenced by id). The renderer solves the geometry exactly — you never compute coordinates for derived points and never get them wrong. Each step has one closed-form solution. Available step kinds: midpoint, point_on_circle, chord, radius, diameter, tangent_at, tangent_from, perpendicular_bisector, perpendicular_from, parallel_through, intersect (line∩line, line∩circle, circle∩circle), polygon_regular, triangle_center (centroid/incenter/circumcenter/orthocenter). Refer to objects by id. Multi-output steps name auto-generated children with UNDERSCORE: a chord with id "ch" produces points "ch_from" and "ch_to" (NOT "ch.from"); polygon_regular with id "hex" produces "hex_v0", "hex_v1", … . If you need a specific name (to label or reference later), pass `endpoints: { from, to }` / `vertexIds: [...]` / `touchId` / `footId` etc. directly. Use display.showCoords / showLength / labels / colors to control text. Reserve show_geometry for free-form sketches that don\'t fit a construction language.',
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string' },
