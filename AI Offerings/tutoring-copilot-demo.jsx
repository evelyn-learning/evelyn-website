...
// AI Tutoring Co-Pilot Demo
// Shows how AI can assist tutors in real-time during sessions
...
const SCENARIO_DATA = {
  ...
  sessionHistory: [
    { role: 'student', message: "I don't understand why we need to factor these equations", time: '2:00 PM' },
    { role: 'tutor', message: "Great question! Factoring helps us find where the parabola crosses the x-axis. Let me show you...", time: '2:01 PM' },
    { role: 'student', message: "But what about when the numbers are weird? Like x² + 5x + 6?", time: '2:03 PM' }
  ]
};

export default function TutoringCoPilot() {
  const [sessionHistory, setSessionHistory] = useState(SCENARIO_DATA.sessionHistory);
  const [studentMessage, setStudentMessage] = useState('');
  const [tutorMessage, setTutorMessage] = useState('');
  const [aiSuggestions, setAiSuggestions] = useState([]);
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [showStudentInsights, setShowStudentInsights] = useState(true);
