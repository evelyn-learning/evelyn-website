# Lesson brief — m8math · Unit 4 Topic 4 · One, None or Infinitely Many Solutions

You are authoring ONE lesson-plan seed file. Follow the fan-out contract
exactly: `.superpowers/sdd/2026-09-grades-6-8-wave/m8math-FANOUT-CONTRACT.md`.
Read it in full before you write anything. It carries the segment recipe, the
lint requirements, the TypeScript shape and the hard rules.

## Your lesson — these values are authoritative, use them verbatim

| Field | Value |
|---|---|
| Unit | 4 — Linear Equations in One Variable |
| Topic index in unit | 4 |
| Title | One, None or Infinitely Many Solutions |
| Slug | `one-none-or-infinitely-many-solutions` |
| Standard | `8.EE.C.7a` |
| Seed file | `apps/tutor/src/lib/tutor/lesson-plan/seeds/m8math-u4-one-none-or-infinitely-many-solutions.ts` |
| Export symbol | `SEED_M8MATH_U4_ONE_NONE_OR_INFINITELY_MANY_SOLUTIONS` |
| Plan id | `evelyn.ms.m8math.one-none-or-infinitely-many-solutions.v1` |
| Learning-objective id | `m8math.one-none-or-infinitely-many-solutions` |
| `los[0].standard` | `M8MATH-4.4` |
| `cedUnit` / `cedTopic` / `cedTitle` | `'4'` / `'4.4'` / `'One, None or Infinitely Many Solutions'` |
| `prerequisites` | ["m8math.equations-with-rational-coefficients"] |
| `followUps` | ["m8math.solutions-of-systems-as-intersection-points"] |

## Scope — teach exactly this, and nothing adjacent

Transform an equation until it reads x = a, a = a, or a = b, and conclude one solution, infinitely many, or none; explain why 2x + 6 = 2(x + 3) is always true and 2x + 6 = 2x + 5 never; numeric item: "for what value of c does 3x + c = 3x + 7 have infinitely many solutions". Withholds: nothing further; `alg1-u2-multi-step-equations.ts` repeats the classification as HS review.

The neighbouring topics in this course own their own lessons; do not teach
theirs. The signed curriculum
(`.superpowers/sdd/2026-09-grades-6-8-wave/m8math-CURRICULUM.md`) lists every
sibling topic and an "Explicitly excluded" section naming what belongs to the
next grade — read both and stay inside your row.

## Salvage

none

## Hard rules

- Write ONLY `m8math-u4-one-none-or-infinitely-many-solutions.ts`. Do not touch `store.ts`, do not register anything, do not
  commit, do not push, do not run a deploy, do not touch a database, do not
  dispatch subagents. The controller registers all lessons in one batched edit.
- Every number must be arithmetically correct. Check each worked example and
  each answer by hand before you report.
- Age-appropriate for thirteen- and fourteen-year-olds (Grade 8).
- Report back: the file you wrote, and any place the contract or this brief was
  wrong or ambiguous. Do not paste the file's contents into your reply.
